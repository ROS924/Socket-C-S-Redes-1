...
def start_udp_server():
    ...
    threading.Thread(target=handle_udp_client,
                     args=(server_socket_udp,)).start()
...
def handle_connection(client_socket, source_addresses):
    ...
    if message.startswith("CONNECT"):
        ...
        message = "Connected with " + \
            str(len(source_addresses)) + \
            " machines with these addresses " + str(source_addresses)
        ...
        if (len(get_ips_for_file(filename)) > copies):
            ...
            destination_list = random.sample(
                source_addresses, len(get_ips_for_file(filename))-copies)
            print("Destination list passed", destination_list)

            for element in destination_list:
                print("destination about to go")
                send_modify_message(filename.split(
                    ".")[0] + "-proxy-server.txt", element)
